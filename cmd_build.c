...
int cmd_build_project(const char *path, const char *out_path, const char *target_override, const char *offline_cache, int include_runtime) {
    ...
    if (proj.cache_root[0]) {
        ...
        snprintf(cache_file, sizeof(cache_file), "%s/project_bundle.snbc", proj.cache_root);
    ...
    printf("Compiled project %s (%s) -> %s (target: %s)\n",
           path, include_runtime ? "bundled binary + embedded runtime" : "bundled binary dependencies",
